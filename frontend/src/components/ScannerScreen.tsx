...
type ScanResult = { scan_id?: string; diagnosis: string; confidence: string; symptoms: string[]; remedies: string[] };
type ScanHistoryItem = { scan_id: string; diagnosis: string; confidence: string; symptoms: string[]; remedies: string[]; image_base64: string; language: string; created_at: string };
...
export default function ScannerScreen() {
  ...
  const analyze = async () => {
    ...
    try {
      const data = await apiRequest<ScanResult>("/ai/scan", {
        method: "POST",
        body: JSON.stringify({ image_base64: `data:${mime};base64,${base64}`, mime_type: mime, language }),
      }, token);
      setResult(data);
      void loadHistory();
    } catch (e) {
  ...
  const openHistoryItem = (item: ScanHistoryItem) => {
    ...
    setBase64(null);
    setResult({ scan_id: item.scan_id, diagnosis: item.diagnosis, confidence: item.confidence, symptoms: item.symptoms, remedies: item.remedies });
    setError("");
  };
        ...
        {result && <View testID="scanner-result" style={[styles.result, { backgroundColor: c.card, borderColor: c.border }]}>
          <View style={styles.resultHeading}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={[styles.diagnosis, { color: c.text }]}>{result.diagnosis}</Text>
            </View>
            <Text style={[styles.confidence, { color: c.brand }]}>{result.confidence}</Text>
          </View>
            ...
            history.map((item) => (
              ...
              >
                ...
                <View style={styles.historyBody}>
                  <Text numberOfLines={1} style={[styles.historyDiag, { color: c.text }]}>{item.diagnosis}</Text>
...
const styles = StyleSheet.create({
  ...
  confidence: { fontSize: 12, fontWeight: "800", maxWidth: 75, textAlign: "right" },
